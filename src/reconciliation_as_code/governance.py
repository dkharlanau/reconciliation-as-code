...
def apply_exception_governance(
    ...
    result["checks"].append(policy_check)

    failed_errors = [
        item for item in result["checks"] if item["severity"] == "error" and item["status"] == "failed"
    ]
    failed_warnings = [
        item for item in result["checks"] if item["severity"] == "warning" and item["status"] == "failed"
    ]
    result["status"] = "failed" if failed_errors else "passed"
    result["summary"]["checks_total"] = len(result["checks"])
    result["summary"]["checks_failed"] = len(failed_errors)
    result["summary"]["warnings_failed"] = len(failed_warnings)
    result["summary"]["accepted_exceptions"] = len(used)
    result["summary"]["unused_exceptions"] = len(unused)
    result["summary"]["expired_exceptions"] = len(expired_entries)
    result["inputs"]["exceptions"] = {
        "path": str(policy_path.name),
...
def run_reconciliation_with_governance(
    ...
    if policy is None or policy_path is None:
        return run_reconciliation(spec, base_dir=base, spec_path=spec_path)
    ...
    result = run_reconciliation(execution_spec, base_dir=base, spec_path=spec_path)
