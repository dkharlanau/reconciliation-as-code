...
AGENT_MANIFEST = ROOT / "docs" / "agent-manifest.json"
DEFAULT_OUTPUT = ROOT / "build" / "docs-site"
OUTPUT_MARKER = ".rac-docs-site-output"
...
def page_use_case(item: dict, data: dict) -> str:
    ...
    structured = {"@context":"https://schema.org","@type":"TechArticle","headline":item["title"],"description":item["description"],"url":canonical,"isPartOf":{"@type":"WebSite","name":"Reconciliation as Code","url":base}}
...
def prepare_output(output: Path) -> Path:
    ...
    forbidden = {Path(output.anchor), Path.home().resolve(), ROOT, ROOT.parent}
    if output in forbidden:
        raise ValueError(f"refusing unsafe documentation output path: {output}")
    if output.exists() and not output.is_dir():
        raise ValueError(f"documentation output is not a directory: {output}")
    if output.exists():
        entries = list(output.iterdir())
        if entries and not (output / OUTPUT_MARKER).is_file():
            raise ValueError(f"refusing to replace unmarked documentation directory: {output}")
    ...
    (output / OUTPUT_MARKER).write_text("Generated by scripts/build_docs_site.py.\n", encoding="utf-8")
    return output
...
def build(output: Path) -> None:
    ...
    shutil.copy2(AGENT_MANIFEST, output/"agent-manifest.json")
...
def main()->int:
    ...
    args=parser.parse_args(); output=args.output.expanduser().resolve(); build(output); print(json.dumps({"output":str(output),"status":"built"})); return 0
